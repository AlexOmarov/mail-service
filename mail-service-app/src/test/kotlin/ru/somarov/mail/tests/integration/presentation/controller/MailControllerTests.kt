...
class MailControllerTests : BaseIntegrationTest() {
    ...
    fun `When create mail request comes then respond with mail data`() {
        ...
        StepVerifier.create(ex.returnResult(getStandardResponseMailResponseClass()).responseBody)
            .expectNextMatches { standardResponse -> standardResponse.response.mail.text == text }
            .verifyComplete()
    }
    ...
    fun `When create mail request comes then store it in database`() {
        ...
        val ex = webClient.post().uri("/mails")
            ...
            .exchange().expectStatus().isOk
        val response = ex.returnResult(getStandardResponseMailResponseClass()).responseBody.blockFirst()!!.response
    ...
    fun `When get mail request comes then return valid mail data`() {
        ...
        val ex = webClient.get().uri("/mails/${mail.id}")
            ...
            .exchange().expectStatus().isOk
        val response =
            ex.returnResult(getStandardResponseMailResponseClass()).responseBody.blockFirst()!!.response
    ...
    private fun getStandardResponseMailResponseClass(): ParameterizedTypeReference<StandardResponse<MailResponse>> {
        return object : ParameterizedTypeReference<StandardResponse<MailResponse>>() {}
    }
}
